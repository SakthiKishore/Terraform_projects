...
resource "null_resource" "call_install" {
  depends_on = [aws_iam_policy.alb-ingress]
  triggers = {
    always_run = module.eks.cluster_version
  }
...
resource "aws_iam_role_policy_attachment" "eks-ng-alb-ingress_policy" {
  policy_arn = aws_iam_policy.alb-ingress.arn
  role       = "eks-dev-22021102612454365310000000b"
}
